...
Page({
    ...
    onLoad(opt) {
		...
		let dialogueLog = common.LS.get('dialogueLog_boast');
		this.setData({
	...
	sendClickMsg(){
		...
		if(userinfo_count['boast_msg']['count']>=userinfo_count['restrict_count']['kuakua']){
			wx.showModal({
	...
	getInvokeMsg(text){
		...
		Api.invokeBoast({
			...
			if(res.success){
				...
				let userinfo_count = common.LS.get('userinfo_count');
				userinfo_count['boast_msg']['count']=userinfo_count['boast_msg']['count']+1
				common.LS.put('userinfo_count',userinfo_count);
				common.LS.put('dialogueLog_boast',dialogueLog);
				this.setData({
					dialogueLog:dialogueLog
				})
				this.scrollBottom()
